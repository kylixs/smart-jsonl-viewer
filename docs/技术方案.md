# JSONL Viewer Chrome 扩展 - 实施方案

## 一、项目概述

### 核心功能
1. 展开/折叠 JSON Lines 和 JSON 层次结构
2. 智能递归解码复杂字符串（`\n`、`\"`等转义字符）
3. 支持查找过滤（原始内容 + 解码后内容）
4. 两种过滤模式：按行过滤 / 按叶子节点过滤

### 使用场景
- **场景 A**：打开本地 `.jsonl` 文件查看
- **场景 B**：自动美化网页中的 JSONL API 响应（类似 JSONView）

---

## 二、技术架构

### 技术栈
- **框架**：Vue 3 + TypeScript
- **构建工具**：Vite
- **UI 组件库**：无（使用原生 CSS，保持轻量）
- **状态管理**：Pinia（轻量级）
- **虚拟列表**：vue-virtual-scroller（可选，如需后续支持大文件）

### Chrome 扩展架构

```
chrome-extension/
├── manifest.json              # V3 清单文件
├── src/
│   ├── content/               # 内容脚本（拦截网页 JSONL）
│   │   └── content.ts
│   ├── viewer/                # 查看器主界面
│   │   ├── App.vue
│   │   ├── main.ts
│   │   ├── components/
│   │   │   ├── JsonLineItem.vue      # 单行 JSON 组件
│   │   │   ├── JsonTree.vue          # JSON 树形结构
│   │   │   ├── StringDecoder.vue     # 字符串解码器
│   │   │   └── SearchFilter.vue      # 搜索过滤器
│   │   ├── stores/
│   │   │   └── jsonlStore.ts         # 数据状态管理
│   │   └── utils/
│   │       ├── decoder.ts            # 递归解码逻辑
│   │       ├── filter.ts             # 过滤逻辑
│   │       └── parser.ts             # JSONL 解析
│   ├── background/            # 后台脚本
│   │   └── background.ts
│   └── popup/                 # 弹出窗口（可选）
│       └── popup.vue
├── public/
│   ├── icons/                 # 扩展图标
│   └── viewer.html            # 查看器 HTML 模板
└── vite.config.ts
```

---

## 三、核心功能设计

### 3.1 数据结构

#### JsonLineNode（单行 JSONL 数据）
```typescript
interface JsonLineNode {
  id: string                      // 唯一标识
  lineNumber: number              // 原始行号
  rawContent: string              // 原始 JSON 字符串（永不改变）
  parsedData: any                 // 解析后的对象
  isExpanded: boolean             // 展开状态
  decodedStrings: Map<string, DecodedString>  // 解码字符串缓存
  decodedText: string             // 解码后的文本缓存（用于搜索）
  matchedPath?: string            // JSONPath 匹配时的路径
}
```

#### JsonNode（JSON 树节点）
```typescript
interface JsonNode {
  key: string
  value: any
  type: 'object' | 'array' | 'string' | 'number' | 'boolean' | 'null'
  isExpanded: boolean
  children?: JsonNode[]
  depth: number
  isDecodable: boolean            // 是否为可解码的 JSON 字符串
  decodedValue?: any              // 解码后的值
}
```

### 3.2 智能字符串解码

#### 核心设计原则

**自动解码（autoDecodeEnabled）**：
- 全局 UI 偏好设置，控制默认渲染方式和搜索行为
- **所看即所得**：用户看到什么，搜索就在什么内容中进行

#### 两阶段解码策略

**阶段 1：预计算缓存（parser.ts）**
```typescript
export function extractDecodedText(parsedData: any): string {
  let hasDecoding = false  // 标记是否需要解码
  const texts: string[] = []

  function traverse(value: any, depth: number = 0) {
    if (depth > MAX_RECURSION_DEPTH) return

    if (typeof value === 'string') {
      const hasEscape = value.includes('\\')
      const looksLikeJson = value.trim().startsWith('{') || value.trim().startsWith('[')

      if (!hasEscape && !looksLikeJson) {
        texts.push(value)  // 普通字符串，无需解码
        return
      }

      // 处理转义字符
      if (hasEscape) {
        const decoded = unescapeString(value)
        if (decoded !== value) {
          hasDecoding = true
          texts.push(decoded)
        }
      }

      // 尝试解析嵌套 JSON
      if (looksLikeJson && value.length < 100000) {
        try {
          const nested = JSON.parse(value)
          hasDecoding = true
          traverse(nested, depth + 1)
        } catch {}
      }
    } else if (typeof value === 'object') {
      // 递归遍历对象/数组
      if (Array.isArray(value)) {
        value.forEach(item => traverse(item, depth + 1))
      } else {
        for (const key in value) {
          traverse(value[key], depth + 1)
        }
      }
    } else {
      texts.push(String(value))
    }
  }

  traverse(parsedData)

  // 关键优化：如果不需要解码，返回空字符串
  if (!hasDecoding) {
    return ''  // 节省内存，不重复存储
  }

  return texts.join(' ')
}
```

**阶段 2：按需渲染（StringDecoder.vue）**
```typescript
export function smartDecode(value: any, depth: number = 0): DecodedResult {
  if (depth > MAX_RECURSION_DEPTH || typeof value !== 'string') {
    return { type: 'primitive', value }
  }

  // 检测是否为 JSON 字符串
  if (isJsonString(value)) {
    try {
      const parsed = JSON.parse(value)
      return {
        type: 'json',
        original: value,
        decoded: smartDecode(parsed, depth + 1),
        displayMode: 'decoded'
      }
    } catch {
      return { type: 'string', value, displayValue: unescapeString(value) }
    }
  }

  // 处理包含实际换行符等特殊字符的字符串
  if (value.includes('\n') || value.includes('\t') || value.includes('\r')) {
    const formatted = formatXmlIfNeeded(value)  // XML/HTML 格式化
    return { type: 'string', value, displayValue: formatted }
  }

  // 处理转义字符串
  const unescaped = unescapeString(value)
  if (unescaped !== value) {
    return { type: 'string', value, displayValue: unescaped }
  }

  return { type: 'string', value }
}
```

#### 显示模式控制

**全局控制**：
- 用户切换 `autoDecodeEnabled`，所有节点立即刷新
- 监听全局设置变化，强制更新所有组件：
  ```typescript
  watch(() => store.autoDecodeEnabled, (newValue) => {
    displayMode.value = newValue ? 'decoded' : 'original'
  })
  ```

**节点级临时切换**：
- 用户点击单个节点的切换按钮，临时覆盖全局设置
- 下次全局设置变化时，被强制刷新

### 3.3 高级过滤功能

#### 三种搜索模式

**1. 模糊匹配（Fuzzy）**
```typescript
function matchString(text: string, keyword: string, searchMode: 'fuzzy'): boolean {
  return text.includes(keyword)  // 简单包含即可
}
```

**2. 完整匹配（Exact）**
```typescript
function matchString(text: string, keyword: string, searchMode: 'exact'): boolean {
  const regex = new RegExp(`\\b${escapeRegex(keyword)}\\b`, 'i')
  return regex.test(text)  // 完整单词边界匹配
}
```

**3. JSONPath 查询**
```typescript
function evaluateJsonPath(obj: any, path: string): any {
  // 支持语法：
  // $.user.name              # 点号访问
  // $.data[0]                # 数组索引
  // $.items[*]               # 数组通配符
  // $..content               # 递归下降

  if (path.includes('..')) {
    return evaluateRecursivePath(obj, path)  // 递归查找
  }

  // 普通路径解析
  const parts = path.substring(1).split(/\.|\\[/).map(p => p.replace(/\\]$/, ''))
  let current = obj
  for (const part of parts) {
    if (part === '*') {
      return Array.isArray(current) ? current : null
    }
    if (/^\d+$/.test(part)) {
      current = current[parseInt(part)]
    } else {
      current = current[part]
    }
    if (current === undefined) return null
  }
  return current
}
```

#### 两种过滤范围

**1. 按行过滤（Line Mode）**
```typescript
function filterByLine(
  lines: JsonLineNode[],
  keyword: string,
  searchMode: SearchMode,
  autoDecodeEnabled: boolean
): JsonLineNode[] {
  return lines.filter(line => {
    // 所看即所得：根据自动解码设置选择搜索源
    const searchText = (autoDecodeEnabled && line.decodedText)
      ? line.decodedText
      : line.rawContent

    return matchString(searchText.toLowerCase(), keyword, searchMode)
  })
}
```

**2. 按节点过滤（Node Mode）**
```typescript
function filterByNode(
  lines: JsonLineNode[],
  keyword: string,
  searchMode: SearchMode,
  autoDecodeEnabled: boolean
): JsonLineNode[] {
  return lines.map(line => {
    const filteredData = filterNodes(line.parsedData, keyword, searchMode, autoDecodeEnabled)
    if (filteredData === null) return null

    return { ...line, parsedData: filteredData }
  }).filter(line => line !== null)
}

function filterNodes(node: any, keyword: string, ...): any {
  // 递归过滤，只保留匹配的叶子节点及其路径
  if (typeof node !== 'object') {
    // 叶子节点：检查是否匹配
    return searchInObject(node, keyword, ...) ? node : null
  }

  if (Array.isArray(node)) {
    const filtered = node.map(item => filterNodes(item, ...)).filter(item => item !== null)
    return filtered.length > 0 ? filtered : null
  }

  const result: any = {}
  let hasMatch = false
  for (const key in node) {
    if (matchString(key.toLowerCase(), keyword, ...)) {
      result[key] = node[key]  // 键名匹配
      hasMatch = true
    } else {
      const filtered = filterNodes(node[key], ...)
      if (filtered !== null) {
        result[key] = filtered
        hasMatch = true
      }
    }
  }
  return hasMatch ? result : null
}
```

### 3.4 本地文件读取

#### 增量解析策略（100MB+ 大文件优化）

**核心思路**：首屏快速显示，后台异步解析

```typescript
// parser.ts
export function parseJsonLinesIncremental(
  text: string,
  initialBatchSize: number = 100,
  onProgress?: (lines: JsonLineNode[], isComplete: boolean) => void
): JsonLineNode[] {
  const allLines = text.split('\n').filter(line => line.trim())
  const result: JsonLineNode[] = []

  // 第1步：立即解析首批100行
  for (let i = 0; i < Math.min(initialBatchSize, allLines.length); i++) {
    const parsedData = JSON.parse(allLines[i])
    result.push({
      id: `line-${i}`,
      lineNumber: i + 1,
      rawContent: allLines[i],
      parsedData,
      isExpanded: false,
      decodedStrings: new Map(),
      decodedText: extractDecodedText(parsedData)
    })
  }

  // 第2步：回调首批数据，立即显示
  if (onProgress) {
    onProgress([...result], allLines.length <= initialBatchSize)
  }

  // 第3步：后台解析剩余行（每批500行）
  if (allLines.length > initialBatchSize) {
    const parseRemaining = (startIndex: number) => {
      const batchSize = 500
      const endIndex = Math.min(startIndex + batchSize, allLines.length)

      for (let i = startIndex; i < endIndex; i++) {
        const parsedData = JSON.parse(allLines[i])
        result.push({
          id: `line-${i}`,
          lineNumber: i + 1,
          rawContent: allLines[i],
          parsedData,
          isExpanded: false,
          decodedStrings: new Map(),
          decodedText: extractDecodedText(parsedData)
        })
      }

      // 通知进度更新
      if (onProgress) {
        const isComplete = endIndex >= allLines.length
        onProgress([...result], isComplete)
      }

      // 继续解析下一批
      if (endIndex < allLines.length) {
        if (typeof requestIdleCallback !== 'undefined') {
          requestIdleCallback(() => parseRemaining(endIndex))
        } else {
          setTimeout(() => parseRemaining(endIndex), 0)
        }
      }
    }

    // 启动后台解析
    if (typeof requestIdleCallback !== 'undefined') {
      requestIdleCallback(() => parseRemaining(initialBatchSize))
    } else {
      setTimeout(() => parseRemaining(initialBatchSize), 0)
    }
  }

  return result
}
```

**性能优化点**：
1. **首批立即显示**：100 行在 50ms 内完成，用户无感知
2. **requestIdleCallback**：利用浏览器空闲时间解析
3. **批量处理**：每批 500 行，避免长时间阻塞
4. **增量渲染**：每批完成后立即更新 UI

#### 文件上传（拖放支持）

```typescript
// App.vue
const handleFileDrop = async (event: DragEvent) => {
  event.preventDefault()
  const files = event.dataTransfer?.files
  if (!files || files.length === 0) return

  const file = files[0]
  await loadFileContent(file)
}

const handleFileUpload = async (event: Event) => {
  const input = event.target as HTMLInputElement
  if (!input.files || input.files.length === 0) return

  const file = input.files[0]
  await loadFileContent(file)
}

const loadFileContent = async (file: File) => {
  const text = await file.text()

  // 自动检测并增量解析
  const { type, data } = parseTextIncremental(text, (lines, isComplete) => {
    // 进度回调：实时更新界面
    store.updateLines(lines)
    if (isComplete) {
      console.log('文件解析完成')
    }
  })

  if (type === 'invalid') {
    alert('无效的 JSONL/JSON 文件')
  }
}
```

### 3.5 网页 JSONL 拦截

#### 拦截条件判断（基于 Content-Type 和 URL 后缀）

**核心策略**：通过 Content-Type 或 URL 文件后缀快速判断是否应该拦截

```typescript
// content.ts

// 检测 URL 文件后缀（本地文件或远程 URL）
function isJsonUrl(url: string): boolean {
  return /\.(json|jsonl|ndjson)(\?|#|$)/i.test(url)
}

// 检测页面内容类型（仅 application/json，避免误判）
function detectContentType(): boolean {
  return document.contentType.includes('application/json')
}

// 判断是否应该拦截（两个条件满足任一即可）
function shouldIntercept(): boolean {
  const urlMatch = isJsonUrl(location.href)
  const contentTypeMatch = detectContentType()
  return urlMatch || contentTypeMatch
}

// 只有匹配时，才执行拦截逻辑
if (shouldIntercept()) {
  // ... 拦截逻辑
}
```

#### 快速加载策略（Background Fetch + 2秒超时提示）

**问题**：大文件等待浏览器构建 DOM 很慢，100MB 文件需要 5-10 秒

**解决方案**：
1. 优先通过 background script 直接 fetch 文件（需要用户授权 file:// 权限）
2. 设置 2 秒超时：超时显示非侵入式提示框，引导用户授权或打开插件首页
3. 如果 fetch 成功，直接替换页面；失败则等待 DOM 加载

```typescript
// 尝试通过 background script fetch 文件（5秒超时）
async function tryBackgroundFetch(url: string): Promise<string | null> {
  try {
    const messagePromise = chrome.runtime.sendMessage({
      type: 'FETCH_FILE',
      url: url
    })

    const timeoutPromise = new Promise<null>((resolve) => {
      setTimeout(() => resolve(null), 5000)
    })

    const response = await Promise.race([messagePromise, timeoutPromise])

    if (response && response.success) {
      console.log('✅ background 读取成功')
      return response.content
    } else {
      return null
    }
  } catch (error) {
    return null
  }
}

// 主拦截逻辑
async function tryEarlyIntercept() {
  const isFileProtocol = location.href.startsWith('file://')

  // 🚀 优先尝试 background fetch
  const fetchPromise = tryBackgroundFetch(location.href)

  // ⏱️ 设置 2 秒超时：如果 2 秒内没有完成，显示提示
  const timeoutPromise = new Promise<string>((resolve) => {
    setTimeout(() => resolve('timeout'), 2000)
  })

  const result = await Promise.race([fetchPromise, timeoutPromise])

  if (result === 'timeout') {
    // 超时了，显示小提示框（非侵入式）
    showTip(isFileProtocol)

    // 继续等待 fetch 完成
    const content = await fetchPromise
    if (content !== null) {
      // 如果最终成功了，替换页面
      replaceWithViewer(content)
    } else {
      // fetch 失败，等待 DOM 加载完成后再替换
      waitForDOMAndReplace()
    }
  } else if (result !== null) {
    // 2秒内成功了，直接替换
    replaceWithViewer(result)
  } else {
    // 快速失败（没有权限），显示提示并等待 DOM
    showTip(isFileProtocol)
    waitForDOMAndReplace()
  }
}
```

#### MutationObserver 监听 Body 创建

**作用**：在 document_start 阶段，body 可能还未创建，需要监听其创建时机

```typescript
if (document.body) {
  // body 已存在，立即拦截
  tryEarlyIntercept()
} else {
  // body 还未创建，监听 document 变化
  const observer = new MutationObserver(() => {
    if (document.body && !handled) {
      console.log('MutationObserver 检测到 body 创建')
      tryEarlyIntercept()

      // 如果已处理，停止监听
      if (handled) {
        observer.disconnect()
      }
    }
  })

  observer.observe(document.documentElement, {
    childList: true,
    subtree: true
  })
}
```

#### 非侵入式提示框（右下角引导）

**作用**：当快速加载失败或超时时，显示提示引导用户

```typescript
function showTip(isFileProtocol: boolean) {
  const tipDiv = document.createElement('div')
  tipDiv.innerHTML = `
    <div style="position: fixed; bottom: 20px; right: 20px; ...">
      <h3>🚀 JSONL Viewer</h3>
      <p>检测到大文件加载较慢，您可以：</p>
      <button id="open-viewer-btn">✨ 打开插件首页快速加载</button>
      ${isFileProtocol ? `
        <button id="guide-permission-btn">⚡ 授权文件访问（一劳永逸）</button>
      ` : ''}
      <p>或继续等待浏览器加载完成</p>
    </div>
  `
  document.body.appendChild(tipDiv)

  // 10秒后自动隐藏
  setTimeout(() => tipDiv.remove(), 10000)
}
```

#### 替换页面为查看器

```typescript
function replaceWithViewer(text: string) {
  // 创建 iframe，添加 autoload 参数告知是自动加载模式
  const iframe = document.createElement('iframe')
  iframe.src = chrome.runtime.getURL('index.html') + '?autoload=true'
  iframe.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; border:none; z-index:999999;'

  // 清空页面并插入 iframe
  document.body.innerHTML = ''
  document.body.appendChild(iframe)

  // 等待 iframe 加载完成后传递数据
  iframe.onload = () => {
    iframe.contentWindow?.postMessage({
      type: 'LOAD_JSONL',
      data: text
    }, '*')
  }
}
```

**性能对比**：
- **无授权**（等待 DOM 加载）：100MB 文件需要 5-10 秒
- **有授权**（background fetch）：100MB 文件 < 2 秒（节省 80%+ 时间）

### 3.6 Manifest 配置

```json
// public/manifest.json
{
  "manifest_version": 3,
  "name": "JSONL Viewer",
  "version": "1.0.0",
  "description": "智能 JSONL 查看器，支持字符串解码和高级过滤",
  "permissions": ["storage"],
  "content_scripts": [{
    "matches": ["<all_urls>"],
    "js": ["content.js"],
    "run_at": "document_end"
  }],
  "web_accessible_resources": [{
    "resources": ["viewer.html"],
    "matches": ["<all_urls>"]
  }]
}
```

---

## 四、UI/UX 设计

### 视觉风格
- **主题**：支持亮色/暗色主题切换
- **配色**：参考 VSCode JSON 语法高亮
  - 对象键：淡蓝色 `#9CDCFE`
  - 字符串：橙色 `#CE9178`
  - 数字：绿色 `#B5CEA8`
  - 布尔值：紫色 `#569CD6`
  - 解码标记：黄色背景 `#FFF3CD`

### 交互设计
1. **行号区域**：显示原始行号，可点击折叠
2. **JSON 树区域**：使用缩进显示层级关系
3. **工具栏**：
   - 搜索框
   - 过滤模式切换
   - 主题切换
   - 全部展开/折叠按钮
   - 导出功能（导出过滤后的数据）

### 示例界面

```
┌─ JSONL Viewer ────────────────────────────────────────┐
│ [📂 打开文件] [🔍 搜索] [⚙️ 设置] [🌙 主题]          │
├───────────────────────────────────────────────────────┤
│ 搜索: [____________] ○行模式 ●节点模式 ☑解码内容   │
├───────────────────────────────────────────────────────┤
│ 1  ▼ { type: "file-history-snapshot", ...           │
│       ├─ type: "file-history-snapshot"               │
│       ├─ messageId: "0050ba14-338f-..."             │
│       └─ snapshot: ▶ {...}                           │
│                                                       │
│ 2  ▼ { parentUuid: null, ...                        │
│       ├─ message: ▼                                  │
│       │   ├─ role: "user"                            │
│       │   └─ content: "Caveat: The messages..."     │
│       │       [🔄 显示解码内容]                      │
│       └─ ...                                         │
│                                                       │
│ 3  ▶ { parentUuid: "6d5b038a...", ...               │
└───────────────────────────────────────────────────────┘
共 1024 行，显示 3 行（过滤后）
```

---

## 五、实施步骤

### Phase 1: 项目搭建（1 天）
- [x] 初始化 Vite + Vue 3 + TypeScript 项目
- [x] 配置 Chrome 扩展开发环境
- [x] 创建基础目录结构
- [x] 设置 ESLint + Prettier

### Phase 2: 核心组件开发（3 天）
- [ ] `JsonLineItem.vue` - 单行 JSON 显示组件
- [ ] `JsonTree.vue` - 递归树形结构组件
- [ ] `StringDecoder.vue` - 字符串解码组件
- [ ] 实现智能解码算法 `decoder.ts`

### Phase 3: 过滤功能（2 天）
- [ ] `SearchFilter.vue` - 搜索过滤 UI
- [ ] `filter.ts` - 实现两种过滤模式
- [ ] 集成解码内容搜索

### Phase 4: Chrome 扩展集成（2 天）
- [ ] Content Script - 拦截网页 JSONL
- [ ] 本地文件上传功能
- [ ] Manifest V3 配置

### Phase 5: UI/UX 优化（1 天）
- [ ] 主题切换
- [ ] 语法高亮
- [ ] 响应式布局
- [ ] 性能优化（虚拟滚动可选）

### Phase 6: 测试与发布（1 天）
- [ ] 功能测试
- [ ] 不同 JSONL 格式兼容性测试
- [ ] 打包并发布到 Chrome Web Store

---

## 六、关键技术难点

### 6.1 递归解码性能
**问题**：深层嵌套的 JSON 字符串递归解析可能导致性能问题

**解决方案**：
- 使用缓存机制，避免重复解析
- 限制最大递归深度（默认 10 层）
- 懒加载：仅在用户展开节点时才解析子节点

### 6.2 大文件处理（未来扩展）
虽然当前不需要，但预留扩展性：
- 使用 `vue-virtual-scroller` 实现虚拟滚动
- 分页加载：每次只加载 100-500 行
- Web Worker 解析大文件

### 6.3 Content Script 注入
**问题**：Content Script 需要访问页面 DOM 但权限受限

**解决方案**：
```typescript
// 使用 iframe 隔离查看器
const iframe = document.createElement('iframe');
iframe.src = chrome.runtime.getURL('viewer.html');
iframe.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; border:none; z-index:999999;';
document.body.innerHTML = '';
document.body.appendChild(iframe);

// 通过 postMessage 传递数据
iframe.onload = () => {
  iframe.contentWindow?.postMessage({
    type: 'LOAD_JSONL',
    data: bodyText
  }, '*');
};
```

---

## 七、关键文件说明

### 核心文件
1. **`manifest.json`** - Chrome 扩展配置文件
2. **`src/viewer/App.vue`** - 主应用入口
3. **`src/utils/decoder.ts`** - 智能解码核心逻辑
4. **`src/utils/filter.ts`** - 过滤算法实现
5. **`src/components/JsonTree.vue`** - 递归树形结构组件

### 配置文件
- `vite.config.ts` - 构建配置（需要配置多入口）
- `tsconfig.json` - TypeScript 配置
- `package.json` - 依赖管理

---

## 八、可选增强功能

1. **导出功能**：导出过滤后的数据为新的 JSONL 文件
2. **历史记录**：记录最近打开的文件
3. **自定义配色**：允许用户自定义语法高亮颜色
4. **快捷键**：支持键盘快捷键（`Ctrl+F` 搜索、`Ctrl+E` 展开全部等）
5. **统计信息**：显示总行数、过滤后行数、JSON 结构统计
6. **Diff 模式**：对比两个 JSONL 文件的差异

---

## 九、预期效果

完成后的扩展将提供：
- ✅ 优雅的 JSON Lines 查看体验
- ✅ 自动识别并美化网页中的 JSONL 响应
- ✅ 智能解码多层嵌套的 JSON 字符串
- ✅ 强大的搜索过滤功能（支持解码后内容搜索）
- ✅ 轻量级（< 500KB）、快速响应
- ✅ 支持亮/暗两种主题

用户可以像使用 JSONView 一样自然地查看 JSONL 数据，无需复制到外部工具进行解码。

---

## 十、自动解码功能设计规则

### 10.1 核心概念

**`autoDecodeEnabled`** 是一个全局UI偏好设置，控制JSON内容的默认渲染方式和搜索行为。

**设计原则：所看即所得**
- 用户在界面上看到什么内容，搜索就在什么内容中进行
- 解码状态影响渲染和搜索，确保两者保持一致

### 10.2 数据结构

```typescript
interface JsonLineNode {
  id: string                      // 唯一标识
  lineNumber: number              // 原始行号
  rawContent: string              // 原始 JSON 字符串（永不改变）
  parsedData: any                 // 解析后的对象
  isExpanded: boolean             // 展开状态
  decodedStrings: Map<string, DecodedString>  // 解码字符串缓存
  decodedText: string             // 解码后的文本缓存（用于搜索）
}
```

#### `decodedText` 字段生成规则

在解析JSON时（`parser.ts`），调用 `extractDecodedText(parsedData)` 生成：

```typescript
/**
 * 提取对象中所有解码后的文本值
 * @returns 如果内容需要解码，返回解码后的文本；否则返回空字符串
 */
export function extractDecodedText(parsedData: any): string {
  let hasDecoding = false  // 标记是否发生了解码

  // 遍历所有值，检测并收集需要解码的内容
  // 1. 检测转义字符（\n, \t, \", \\）
  // 2. 检测嵌套JSON字符串（以 { 或 [ 开头）

  // 如果没有发生任何解码
  if (!hasDecoding) {
    return ''  // 返回空字符串，表示无需解码
  }

  // 发生了解码，返回解码后的完整文本
  return texts.join(' ')
}
```

**判断标准：**
- ✅ **需要解码**（`decodedText` = 解码后文本）：
  - 包含转义字符：`\n`, `\t`, `\"`, `\\`, `\r`
  - 包含嵌套JSON字符串：值以 `{` 或 `[` 开头且可成功解析
- ❌ **无需解码**（`decodedText` = `''`）：
  - 普通字符串、数字、布尔值等基本类型
  - 不包含转义字符或嵌套结构

### 10.3 autoDecodeEnabled 的作用

#### 作用 1：控制UI渲染

在组件中根据 `autoDecodeEnabled` 决定显示内容：

```typescript
// JsonTree.vue / JsonLineItem.vue
const displayValue = computed(() => {
  if (store.autoDecodeEnabled && node.decodedText) {
    // 自动解码开启，且有解码内容
    return node.decodedText
  } else {
    // 自动解码关闭，或无解码内容
    return node.rawContent
  }
})
```

**渲染规则：**
- ✅ **勾选"自动解码"**：
  - 如果 `decodedText` 非空：显示解码后的内容
  - 如果 `decodedText` 为空：显示 `rawContent`（原始内容）
- ❌ **取消"自动解码"**：
  - 始终显示 `rawContent`（原始JSON字符串）

**手动切换：**
- 即使取消"自动解码"，用户仍可点击节点旁边的解码按钮手动查看解码后的内容
- 手动解码不影响全局 `autoDecodeEnabled` 设置

#### 作用 2：控制搜索行为（所看即所得）

在搜索时，根据 `autoDecodeEnabled` 选择搜索的文本源：

```typescript
// filter.ts
function matchInLine(
  line: JsonLineNode,
  keyword: string,
  searchMode: SearchMode,
  autoDecodeEnabled: boolean
): boolean {
  // 所看即所得：根据用户的渲染偏好选择搜索源
  const searchText = (autoDecodeEnabled && line.decodedText)
    ? line.decodedText     // 搜索解码后的内容
    : line.rawContent      // 搜索原始内容

  return matchString(searchText.toLowerCase(), keyword, searchMode)
}
```

**搜索规则：**
- ✅ **勾选"自动解码"**：
  - 如果 `decodedText` 非空：在解码后的内容中搜索
  - 如果 `decodedText` 为空：在 `rawContent` 中搜索
- ❌ **取消"自动解码"**：
  - 始终在 `rawContent` 中搜索

### 10.4 完整工作流程

#### 流程 1：文件加载与解析

```
用户上传 JSONL 文件
    ↓
parser.ts 解析每一行
    ↓
对每个 JsonLineNode:
  1. 解析 JSON → parsedData
  2. 调用 extractDecodedText(parsedData)
     → 生成 decodedText（可能为空）
  3. 保存到 store
```

#### 流程 2：UI 渲染

```
用户查看 JSON 列表
    ↓
组件读取 store.autoDecodeEnabled
    ↓
如果 autoDecodeEnabled = true 且 decodedText 非空:
  → 显示解码后的内容（换行、嵌套结构等）
否则:
  → 显示原始 rawContent（带转义字符的字符串）
```

#### 流程 3：用户切换"自动解码"

```
用户点击"自动解码"复选框
    ↓
调用 store.toggleAutoDecodeEnabled()
    ↓
autoDecodeEnabled 状态翻转
    ↓
Vue 响应式系统触发重新渲染
    ↓
所有组件根据新的 autoDecodeEnabled 值重新计算 displayValue
    ↓
界面立即更新（解码 ↔ 原始）
```

#### 流程 4：搜索过滤

```
用户输入搜索关键字
    ↓
filter.ts 执行搜索
    ↓
对每个 JsonLineNode:
  1. 读取 autoDecodeEnabled 状态
  2. 选择搜索源:
     - autoDecodeEnabled=true 且 decodedText 非空 → 在 decodedText 中搜索
     - 否则 → 在 rawContent 中搜索
  3. 返回匹配结果
    ↓
只显示匹配的行/节点
```

### 10.5 性能优化要点

1. **预计算解码**：
   - 解码操作在文件加载时一次性完成
   - 搜索时无需重新解码，直接使用缓存的 `decodedText`

2. **内存优化**：
   - 如果不需要解码（约70%的数据），`decodedText` 为空字符串
   - 避免重复存储相同内容

3. **响应式渲染**：
   - Vue 的响应式系统确保 `autoDecodeEnabled` 变化时立即重新渲染
   - 无需手动刷新或重新解析数据

### 10.6 用户体验

**场景 A：查看复杂嵌套数据**
```jsonl
{"id": 1, "data": "{\"user\":\"Alice\",\"msg\":\"Line1\\nLine2\"}"}
```

1. **勾选"自动解码"**：
   ```
   id: 1
   data:
     user: "Alice"
     msg: "Line1
           Line2"
   ```
   搜索 "Alice" → ✅ 找到（在解码内容中）

2. **取消"自动解码"**：
   ```
   id: 1
   data: "{\"user\":\"Alice\",\"msg\":\"Line1\\nLine2\"}"
   ```
   搜索 "Alice" → ❌ 找不到（在原始字符串中搜索需要搜 `\"Alice\"`）
   搜索 "\\nLine2" → ✅ 找到（搜索转义序列）

**场景 B：普通数据（无需解码）**
```jsonl
{"id": 2, "name": "Bob", "age": 30}
```

- `decodedText` = `''`（空字符串）
- 无论是否勾选"自动解码"，显示和搜索行为一致
- 搜索 "Bob" → ✅ 始终能找到

### 10.7 实现文件清单

| 文件 | 职责 | 关键函数/变量 |
|------|------|--------------|
| `types.ts` | 定义数据结构 | `JsonLineNode.decodedText`<br>`SearchOptions.autoDecodeEnabled` |
| `decoder.ts` | 解码逻辑 | `extractDecodedText()` |
| `parser.ts` | 解析时生成缓存 | 调用 `extractDecodedText()` |
| `filter.ts` | 搜索逻辑 | `matchInLine()`<br>`filterNodes()` |
| `jsonlStore.ts` | 状态管理 | `autoDecodeEnabled`<br>`toggleAutoDecodeEnabled()` |
| `SearchFilter.vue` | UI控件 | 复选框绑定 `autoDecodeEnabled` |
| `JsonTree.vue`<br>`JsonLineItem.vue` | 渲染组件 | 根据 `autoDecodeEnabled` 选择显示内容 |

### 10.8 测试用例

#### 测试 1：decodedText 生成正确性
```typescript
// 输入
const input1 = { msg: "Hello\\nWorld" }
// 期望：decodedText = "Hello\nWorld"（包含实际换行）

const input2 = { data: "{\"key\":\"value\"}" }
// 期望：decodedText 包含解析后的嵌套内容

const input3 = { name: "Alice", age: 30 }
// 期望：decodedText = ''（无需解码）
```

#### 测试 2：渲染切换
```typescript
// 1. 勾选"自动解码"
store.autoDecodeEnabled = true
// 期望：显示解码后内容（如果有）

// 2. 取消"自动解码"
store.autoDecodeEnabled = false
// 期望：显示原始 rawContent
```

#### 测试 3：搜索行为一致性
```jsonl
{"id": 2, "data": "{\"secret\":\"password\"}"}
```

```typescript
// 场景A：勾选"自动解码"
store.autoDecodeEnabled = true
search("secret")  // ✅ 应该找到（在解码内容中）
search("\\\"secret\\\"")  // ❌ 找不到（不搜原始转义）

// 场景B：取消"自动解码"
store.autoDecodeEnabled = false
search("secret")  // ❌ 找不到（在原始字符串中）
search("\\\"secret\\\"")  // ✅ 应该找到（搜转义序列）
```

---
