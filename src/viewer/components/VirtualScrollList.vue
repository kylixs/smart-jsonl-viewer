 ...
 *
 * 配置参数：
 * - WINDOW_SIZE = 50 行（初始窗口 + 每次加载批次）
 * - BUFFER_SIZE = 50 行（前后各保留50行）
 * - MAX_WINDOW_SIZE = 150 行（50 + 50 + 50）
 * - LOAD_THRESHOLD = 200px（触发加载的距离阈值）
 *
 * 性能权衡：
 * - 窗口太小(如20)：频繁加载，滚动卡顿
 * - 窗口太大(如200)：DOM节点多，渲染慢
 * - 当前配置(50)：平衡性能与流畅度
 */
// ===========================================================
...
const props = withDefaults(defineProps<Props>(), {
  ...
  loadThreshold: 200,       // 加载阈值：距离边界200px时触发
  batchSize: 50,            // 批次大小：每次加载50行（与窗口大小一致）
  maxWindowSize: 150        // 最大窗口：前50 + 当前50 + 后50 = 150行
})
